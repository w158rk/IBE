#include "user_lcl.hpp"
#include<iostream>

#include<config.h>
#include <crypto.h>
#include <sys.h>
#include <utils.h>

using namespace user;

void User::run_get_private_key(char *server_ip, 
								int server_port,
								ID *server_id)
{
	if(server_id == nullptr) {
		server_id = get_id()->father_node;
	}

	/* connect to the server first */
	interface::IComm *comm = get_comm_ptr();
	comm->connect_to_server(server_ip, server_port);

	// QUESTION : is it necessary to run a listening thread for the connection ?	
	 try
	 {
		// get the files from the comm object
		comm_file_main(this, comm->get_read_file(), 
									comm->get_write_file());
	 }
	 catch(const std::exception& e)
	 {
	 	std::cerr << e.what() << '\n';
	 	throw e;
	 }

	/**
	 * the format of the head is:
	 * --------------------------------------
	 * |	type	|	length of payload   |
	 * --------------------------------------
	 * the format of the payload is :
	 * 	--------------------------
	 * 	|	sm4_key 	|	ID   |
	 * 	-------------------------- 
	 */

	/* get the id */
	char *id = get_id()->id;
	size_t id_len = get_id()->length;		// the upper integer of (id_len+1)
	size_t m_len = (size_t)(id_len + SM4_KEY_LEN);
	char *payload = (char *)std::malloc(m_len);

	AppPacket *p_app_packet = new AppPacket; 
	
	/* set the head */
	p_app_packet->set_type(PRIVATE_KEY_REQUEST_TYPE);
	p_app_packet->set_length(m_len);

	/* generate and copy the sm4 key */
	char *key = (char *)std::malloc(SM4_KEY_LEN);
	gen_random_sm4((unsigned char *)key);
	set_sm4_key(key);
	memcpy(payload, key, SM4_KEY_LEN);		//把key复制到p中

	/* copy the id */
	memcpy(payload+SM4_KEY_LEN, id, (size_t)id_len);		
	p_app_packet->set_payload (payload);

	PacketCTX *ctx = new PacketCTX;
	// NOTE : It is the responsibility of packet class to 
	// free every packet-ctx generated by any module

	ctx->set_phase (SEND_APP_PACKET);
	ctx->set_payload_app (p_app_packet);
	ctx->set_dest_id (server_id);

	// this code is a little weird, but it works 
	// to get the mpk into the ctx

	// IBEPublicParameters mpk = NULL;
	// get_mpk_fp(get_mpk_filename(), &mpk);		//从文件中读出sP的值
	

	if(0 == get_packet_ptr()->packet_send(ctx)) {
		throw UserException("wrong when make the packet to send");
	}


}

void User::run_get_mpk(char *server_ip, 
						int server_port,
						ID *server_id)
{
	if(server_id == nullptr) {
		server_id = get_id()->father_node;
	}

	/* connect to the server first */
	interface::IComm *comm = get_comm_ptr();
	comm->connect_to_server(server_ip, server_port);

	// QUESTION : is it necessary to run a listening thread for the connection ?	
	 try
	 {
		// get the files from the comm object
		comm_file_main(this, comm->get_read_file(), 
									comm->get_write_file());
	 }
	 catch(const std::exception& e)
	 {
	 	std::cerr << e.what() << '\n';
	 	throw e;
	 }

	/**
	 * the format of the head is:
	 * --------------------------------------
	 * |	type	|	length of payload   |
	 * --------------------------------------
	 * the format of the payload is :
	 * 	--------------------------
	 * 	|	sm4_key 	|	ID   |
	 * 	-------------------------- 
	 */

	/* get the id */
	char *id = get_id()->id;
	size_t id_len = get_id()->length;		// the upper integer of (id_len+1)
	size_t m_len = id_len;
	char *payload = (char *)std::malloc(m_len);

	AppPacket *p_app_packet = new AppPacket; 
	
	/* set the head */
	p_app_packet->set_type(MPK_REQUEST_TYPE);
	p_app_packet->set_length(m_len);

	/* copy the id */
	memcpy(payload, id, (size_t)id_len);		
	p_app_packet->set_payload (payload);

	PacketCTX *ctx = new PacketCTX;
	// NOTE : It is the responsibility of packet class to 
	// free every packet-ctx generated by any module

	ctx->set_phase (SEND_APP_PACKET);
	ctx->set_payload_app (p_app_packet);
	ctx->set_dest_id (server_id);

	// this code is a little weird, but it works 
	// to get the mpk into the ctx

	// IBEPublicParameters mpk = NULL;
	// get_mpk_fp(get_mpk_filename(), &mpk);		//从文件中读出sP的值
	

	if(0 == get_packet_ptr()->packet_send(ctx)) {
		throw UserException("wrong when make the packet to send");
	}


}